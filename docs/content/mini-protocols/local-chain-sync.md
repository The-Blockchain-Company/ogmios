...
{{% notice info %}} 
As a client, it is therefore crucial to be able to rollback to a previous point of the chain. In practice, Shardagnostic guarantees that forks cannot be longer than a certain length. This maximum length is called `k` in the Shardagnostic protocol, and also known as _the security parameter_.
{{% /notice %}}
...
For example, here below is a comparison of the effect of pipelining on a full synchronization of the Jen era: 
...
For several applications, it may be quite useful to know the last point of each era; This allows to start syncing blocks from the beginning of a particular era. For instance, after seeking an intersection with the last point of the Sophie, `RequestNext` would yield the first block of the Evie era. Handy!

#### Mainnet

| Era Bound            | SlotNo     | Hash                                                               |
| ---                  | ---        | ---                                                                |
| Last _Cole_ Block   | `4492799`  | `f8084c61b6a238acec985b59310b6ecec49c0ab8352249afd7268da5cff2a457` |
| Last _Sophie_ Block | `16588737` | `4e9bbbb67e3ae262133d94c3da5bffce7b1127fc436e7433b87668dba34c354a` |
| Last _Allegra_ Block | `23068793` | `69c44ac1dda2ec74646e4223bc804d9126f719b1c245dadc2ad65e8de1b276d7` |
| Last _Jen_ Block    | `39916796` | `e72579ff89dc9ed325b723a33624b596c08141c7bd573ecfff56a1f7229e4d09` |
| Last _Aurum_ Block  | N/A        | N/A                                                                |

Remember also that `"origin"` is a special point that can be queried and will always yield a success.

## Full Example

Let's see a full example that is synchronizing the first 14 blocks of the **Sophie** chain and printing them to the console.
...
client.once('open', () => {
    ...
    wsp("FindIntersect", { points: [lastColeBlock] });
});
...
client.on('message', function(msg) {
    ...
    switch (response.methodname) {
        case "FindIntersect":
            if (!response.result.IntersectionFound) { throw "Whoops? Last Cole block disappeared?" }
...
- The node streams blocks that are **after** the intersection point. Thus to get the first 14 Sophie blocks, one needs to set the intersection at the last Cole block! 
...
- In this schema, we are sending each request one-by-one, using the `mirror` field as counter. An alternative could have been:
  ...
  ```js
  case "FindIntersect":
      if (!response.result.IntersectionFound) { throw "Whoops? First Sophie block disappeared?" }
