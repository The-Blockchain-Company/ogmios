 ...
 */
export type Block = Cole | Sophie | Evie | Jen | Aurum;
/**
 * A block in the Cole era. Most blocks are stanard blocks, but once at the beginning of each era is an additional epoch boundary block (a.k.a. EBB)
 */
export type BlockCole = StandardBlock | EpochBoundaryBlock;
/**
...
/**
 * An Ed25519-BIP32 Cole genesis delegate verification key with chain-code.
 */
...
/**
 * A Blake2b 32-byte digest of a Cole transaction witness set, CBOR-encoded.
 */
export type DigestBlake2BBlockColeBodyTxPayloadWitness = string;
/**
 * A Blake2b 32-byte digest of a Cole delegation payload, CBOR-encoded.
 */
export type DigestBlake2BBlockColeBodyDlgPayload = string;
/**
 * A Blake2b 32-byte digest of a Cole update payload, CBOR-encoded.
 */
export type DigestBlake2BBlockColeBodyUpdatePayload = string;
...
/**
 * A number of entropic, possibly large when summed up.
 */
...
/**
 * An amount, possibly negative, in Entropic (1e6 Entropic = 1 Bcc).
 */
export type EntropicDelta = number;
/**
 * Source of rewards as defined by the protocol parameters.
 */
export type RewardPot = "reserves" | "treasury";
export type UpdateSophie = Null | UpdateProposalSophie;
...
/**
 * A phase-1 monetary script. Timelocks constraints are only supported since Evie.
 */
...
/**
 * Extra attributes carried by Cole addresses (network magic and/or HD payload).
 */
...
export type UpdateAurum = Null | UpdateProposalAurum;
export type Int64 = number;
/**
 * A network target, as defined since the Sophie era.
 */
...
export type Era = "Cole" | "Sophie" | "Evie" | "Jen" | "Aurum";
/**
...
export type Entropics = Entropic[];
...
/**
 * The number of Entropic owned by the stake pool owners. If this value is not at least as large as the 'pledgeRatio', the stake pool will not earn any rewards for the given epoch.
 */
...
export interface Ogmios {
  /**
   * Request next block from the current bcc-node's cursor.
   */
  RequestNext: {
  ...
  /**
   * Ask bcc-node to find an intersection between its local chain and the given points.
   */
  FindIntersect: {
    type: "jsonwsp/request";
    version: "1.0";
  ...
  FindIntersectResponse: {
    ...
    methodname: "FindIntersect";
    /**
     * Intersection found if any, alongside the bcc-node's current tip.
     */
  ...
  "QueryResponse[currentProtocolParameters]": {
    ...
    servicename: "ogmios";
    methodname: "Query";
    result: ProtocolParametersSophie | ProtocolParametersAurum | EraMismatch | QueryUnavailableInCurrentEra;
    /**
     * Any value that was set by a client request in the 'mirror' field.
     */
    reflection?: {
  ...
  "QueryResponse[proposedProtocolParameters]": {
    ...
    methodname: "Query";
    result:
      | ProposedProtocolParametersSophie
      | ProposedProtocolParametersAurum
      | EraMismatch
...
export interface StandardBlock {
  ...
  body: {
    ...
    updatePayload: {
      proposal: Null | UpdateProposalCole;
      votes: Vote[];
    };
  };
}
export interface BlockProof {
  utxo: {
    ...
    witnessesHash: DigestBlake2BBlockColeBodyTxPayloadWitness;
  };
  delegation: DigestBlake2BBlockColeBodyDlgPayload;
  update: DigestBlake2BBlockColeBodyUpdatePayload;
}
...
/**
 * A transaction output. Since Jen, 'value' always return a multi-asset value. Since Aurum, 'datum' is always present (albeit sometimes 'null')
 */
...
export interface UpdateProposalCole {
  body: {
    protocolVersion: ProtocolVersion;
    softwareVersion: SoftwareVersion;
    metadata: {
    ...
    parametersUpdate: ProtocolParametersCole;
  };
  issuer: IssuerVrfVerificationKey;
  signature: IssuerSignature;
}
export interface ProtocolParametersCole {
...
export interface Sophie {
  sophie: BlockSophie;
}
export interface BlockSophie {
  body?: TxSophie[];
...
export interface Rewards {
  [k: string]: EntropicDelta;
}
export interface Withdrawals {
  [k: string]: Entropic;
}
export interface UpdateProposalSophie {
  epoch: Epoch;
  proposal: {
    [k: string]: ProtocolParametersSophie;
  };
}
export interface ProtocolParametersSophie {
...
export interface Jen {
  jen: BlockJen;
}
export interface BlockJen {
  body?: TxJen[];
  headerHash?: DigestBlake2BBlockHeader;
  header?: {
    blockHeight: BlockNo;
...
export interface UpdateProposalAurum {
  epoch: Epoch;
  proposal: {
    [k: string]: ProtocolParametersAurum;
  };
}
export interface ProtocolParametersAurum {
  minFeeCoefficient: NullableUInt64;
  minFeeConstant: NullableUInt64;
  maxBlockBodySize: NullableUInt64;
  maxBlockHeaderSize: NullableUInt64;
...
/**
 * Only in Sophie. Replaced with 'outsideOfValidityInterval' since Evie.
 */
...
/**
 * Since Evie. Replaces 'expiredUtxo'.
 */
...
export interface FeeTooSmall {
  feeTooSmall: {
    requiredFee: Entropic;
    actualFee: Entropic;
  };
}
/**
 * Returns 'Value' since Jen, and 'EntropicDelta' before.
 */
export interface ValueNotConserved {
  valueNotConserved: {
    consumed: EntropicDelta | Value;
    produced: EntropicDelta | Value;
  };
}
 ...
 * Only since Jen.
 */
export interface TriesToForgeBcc {
  triesToForgeBcc: null;
}
...
export interface CollateralTooSmall {
  collateralTooSmall: {
    requiredCollateral: Entropic;
    actualCollateral: Entropic;
  };
}
export interface CollateralIsScript {
  collateralIsScript: Utxo;
}
export interface CollateralHasNonBccAssets {
  collateralHasNonBccAssets: Value;
}
...
/**
 * Returned when trying to evaluate execution units of a pre-Aurum transaction. Note that this isn't possible with Ogmios because transactions are always de-serialized as Aurum transactions.
 */
export interface EvaluationFailureIncompatibleEra {
  /**
   * The era in which the transaction has been identified.
   */
  IncompatibleEra: "Cole" | "Sophie" | "Evie" | "Jen";
}
/**
...
export interface GetNonMyopicMemberRewards {
  nonMyopicMemberRewards: Entropics | Credentials;
}
...
/**
 * Rewards that can be expected assuming a pool is fully saturated. Such rewards are said non-myopic, in opposition to short-sighted rewards looking at immediate benefits. Keys of the map can be either Entropic amounts or account credentials depending on the query.
 */
...
export interface ProposedProtocolParametersSophie {
  [k: string]: ProtocolParametersSophie;
}
export interface ProposedProtocolParametersAurum {
  [k: string]: ProtocolParametersAurum;
}
/**
...
export interface CompactGenesis {
  ...
  slotLength: SlotLength;
  updateQuorum: UInt64;
  maxEntropicSupply: UInt64;
  protocolParameters: ProtocolParametersSophie;
}
/**
...
export interface RewardsProvenance1 {
  ...
  totalRewards: number;
  /**
   * The total amount of staked Entropic during this epoch.
   */
  activeStake: number;
  pools: {
