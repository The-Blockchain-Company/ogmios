...
import { Ogmios, TxId } from '@bcc-ogmios/schema'
import { Query } from '../StateQuery'

/**
 * Submit a serialized transaction. This expects a base16 or base64 CBOR-encoded
 * transaction as obtained from the bcc-cli or bcc-serialization-lib.
 *
        ...
        if ('SubmitSuccess' in result) {
          ...
          if (Array.isArray(SubmitFail)) {
            reject(SubmitFail.map(failure => {
              ...
              } else if (errors.TriesToForgeBcc.assert(failure)) {
                return new errors.TriesToForgeBcc.Error(failure)
              ...
              } else if (errors.CollateralHasNonBccAssets.assert(failure)) {
                return new errors.CollateralHasNonBccAssets.Error(failure)
