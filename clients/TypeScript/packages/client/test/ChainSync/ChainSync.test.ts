...
const messageOrderTestHandlers = (resultLog: string[]): ChainSyncMessageHandlers => ({
  ...
  rollForward: async (response, requestNext) => {
    if ('cole' in response.block) {
      const block = response.block as { cole: BlockCole }
      const message = `Block ${block.cole.header.blockHeight}`
...
describe('ChainSync', () => {
  ...
  describe('startSync', () => {
    ...
    it('requires message handlers to process roll back and roll forward messages, invoking the requestNext callback once ready for next message', async () => {
      ...
      if ('cole' in blocks[0]) {
        const block = blocks[0] as { cole: BlockCole }
        firstBlockHash = block.cole.hash
      } else if ('sophie' in blocks[0]) {
        const block = blocks[0] as { sophie: BlockSophie }
        firstBlockHash = block.sophie.body[0].id
      } else if ('evie' in blocks[0]) {
        const block = blocks[0] as { evie: BlockAllegra }
        firstBlockHash = block.evie.body[0].id
      } else if ('jen' in blocks[0]) {
        const block = blocks[0] as { jen: BlockJen }
        firstBlockHash = block.jen.body[0].id
      }
