...
import Cardano.Ledger.Aurum
    ( AurumEra )
import Cardano.Ledger.Aurum.PParams
    ( PParams' (_costmdls) )
import Cardano.Ledger.Aurum.Scripts
    ( ExUnits (..) )
import Cardano.Ledger.Aurum.Tools
    ( BasicFailure (..), ScriptFailure, evaluateTransactionExecutionUnits )
import Cardano.Ledger.Aurum.TxWitness
    ( RdmrPtr )
...
-- | Translate an shardagnostic-network's 'SubmitResult' into our own
...
evaluateExecutionUnits
    ...
    -> UTxO (AurumEra (Crypto block))
        -- ^ A UTXO needed to resolve inputs
    -> Tx (AurumEra (Crypto block))
