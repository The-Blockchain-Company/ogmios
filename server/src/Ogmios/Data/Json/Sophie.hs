...
module Ogmios.Data.Json.Sophie where
...
import Cardano.Ledger.Keys
    ( KeyRole (..) )
import Cardano.Ledger.Sophie.Constraints
...
import qualified Cardano.Protocol.TOptimum.OCert as TOptimum
...
import qualified Cardano.Ledger.Sophie.Address.Bootstrap as Sh
import qualified Cardano.Ledger.Sophie.BlockChain as Sh
import qualified Cardano.Ledger.Sophie.Delegation.Certificates as Sh
import qualified Cardano.Ledger.Sophie.Genesis as Sh
import qualified Cardano.Ledger.Sophie.LedgerState as Sh
import qualified Cardano.Ledger.Sophie.Metadata as Sh
import qualified Cardano.Ledger.Sophie.PParams as Sh
import qualified Cardano.Ledger.Sophie.Rules.Deleg as Sh
import qualified Cardano.Ledger.Sophie.Rules.Delegs as Sh
import qualified Cardano.Ledger.Sophie.Rules.Delpl as Sh
import qualified Cardano.Ledger.Sophie.Rules.Ledger as Sh
import qualified Cardano.Ledger.Sophie.Rules.Pool as Sh
import qualified Cardano.Ledger.Sophie.Rules.Ppup as Sh
import qualified Cardano.Ledger.Sophie.Rules.Utxo as Sh
import qualified Cardano.Ledger.Sophie.Rules.Utxow as Sh
import qualified Cardano.Ledger.Sophie.Scripts as Sh
...
encodeAddress = \case
    Ledger.AddrBootstrap addr ->
        Cole.encodeAddress (Ledger.unBootstrapAddress addr)
...
encodeBHeader
    :: Crypto crypto
    => SerializationMode
    -> TOptimum.BHeader crypto
    -> Json
encodeBHeader mode (TOptimum.BHeader hBody hSig) = encodeObjectWithMode mode
    [ ( "blockHeight"
      , encodeBlockNo (TOptimum.bheaderBlockNo hBody)
      )
    , ( "slot"
      , encodeSlotNo (TOptimum.bheaderSlotNo hBody)
      )
    , ( "prevHash"
      , encodePrevHash (TOptimum.bheaderPrev hBody)
      )
    , ( "issuerVk"
      , encodeVKey (TOptimum.bheaderVk hBody)
      )
    , ( "issuerVrf"
      , encodeVerKeyVRF (TOptimum.bheaderVrfVk hBody)
      )
    , ( "blockSize"
      , encodeNatural (TOptimum.bsize hBody)
      )
    , ( "blockHash"
      , encodeHash (TOptimum.bhash hBody)
      )
    ]
    [ ( "signature"
    ...
    , ( "nonce"
      , encodeCertifiedVRF (TOptimum.bheaderEta hBody)
      )
    , ( "leaderValue"
      , encodeCertifiedVRF (TOptimum.bheaderL hBody)
      )
    , ( "opCert"
      , encodeOCert (TOptimum.bheaderOCert hBody)
      )
    , ( "protocolVersion"
      , encodeProtVer (TOptimum.bprotver hBody)
      )
    ]
...
encodeGenesis x = encodeObject
    ...
    , ( "maxEntropicSupply"
      , encodeWord64 (Sh.sgMaxEntropicSupply x)
      )
    , ( "protocolParameters"
...
encodeHashHeader
    :: TOptimum.HashHeader crypto
    -> Json
encodeHashHeader =
    encodeByteStringBase16 . CC.hashToBytes . TOptimum.unHashHeader
...
encodeKESPeriod
    ...
    -> Json
encodeKESPeriod =
    encodeWord . TOptimum.unKESPeriod

encodeLedgerFailure
    :: Crypto crypto
    => Sh.LedgerPredicateFailure (SophieEra crypto)
    -> Json
...
encodeOCert
    :: Crypto crypto
    => TOptimum.OCert crypto
    -> Json
encodeOCert x = encodeObject
    [ ( "hotVk"
      , encodeVerKeyKES (TOptimum.ocertVkHot x)
      )
    , ( "count"
      , encodeWord64 (TOptimum.ocertN x)
      )
    , ( "kesPeriod"
      , encodeKESPeriod (TOptimum.ocertKESPeriod x)
      )
    , ( "sigma"
      , encodeSignedDSIGN (TOptimum.ocertSigma x)
      )
    ]
...
encodePrevHash
    :: TOptimum.PrevHash crypto
    -> Json
encodePrevHash = \case
    TOptimum.GenesisHash -> encodeText "genesis"
    TOptimum.BlockHash h -> encodeHashHeader h
...
encodeTx
    ...
    => SerializationMode
    -> Sh.Tx (SophieEra crypto)
    -> Json
encodeTx mode x = encodeObjectWithMode mode
    [ ( "id"
      , encodeTxId (Ledger.txid @(SophieEra crypto) (Sh.body x))
      )
    , ( "body"
...
encodeTxOut
    :: (SophieBased era, Ledger.Value era ~ Coin)
    => Sh.TxOut era
    -> Json
...
encodeUtxoFailure
    :: Crypto crypto
    => Sh.UtxoPredicateFailure (SophieEra crypto)
    -> Json
...
encodeWitnessSet
    :: Crypto crypto
    => Sh.WitnessSet (SophieEra crypto)
    -> Json
