...
module Ogmios.Data.Json.Cole where
...
import qualified Cardano.Chain.Block as By hiding
    ( Proof, signature )
import qualified Cardano.Chain.Cole.API as By
...
encodeEntropic
    ...
    -> Json
encodeEntropic =
    encodeInteger . By.entropicToInteger

encodeEntropicError
    :: By.EntropicError
    -> Json
encodeEntropicError = \case
    By.EntropicOverflow x ->
        encodeObject [ ( "overflow", encodeWord64 x ) ]
    By.EntropicTooLarge x ->
        encodeObject [ ( "tooLarge", encodeInteger x ) ]
    By.EntropicTooSmall x ->
        encodeObject [ ( "tooSmall",  encodeInteger x ) ]
    By.EntropicUnderflow x y ->
        encodeObject [ ( "underflow", encodeList encodeWord64 [x,y] ) ]

encodeEntropicPortion
    :: By.EntropicPortion
    -> Json
encodeEntropicPortion =
    encodeRational . By.entropicPortionToRational
...
encodeProtocolParametersUpdate x = encodeObject
    ...
    , ( "mpcThreshold"
      , encodeMaybe encodeEntropicPortion (By.ppuMpcThd x)
      )
    , ( "heavyDlgThreshold"
      , encodeMaybe encodeEntropicPortion (By.ppuHeavyDelThd x)
      )
    , ( "updateVoteThreshold"
      , encodeMaybe encodeEntropicPortion (By.ppuUpdateVoteThd x)
      )
    , ( "updateProposalThreshold"
      , encodeMaybe encodeEntropicPortion (By.ppuUpdateProposalThd x)
      )
...
encodeSoftforkRule x = encodeObject
    [ ( "initThreshold"
      , encodeEntropicPortion (By.srInitThd x)
      )
    , ( "minThreshold"
      , encodeEntropicPortion (By.srMinThd x)
      )
    , ( "decrementThreshold"
      , encodeEntropicPortion (By.srThdDecrement x)
      )
    ]
...
encodeTxValidationError = \case
    By.TxValidationEntropicError lbl e -> encodeObject
        [ ( "entropicError", encodeObject
            [ ( "label", encodeText lbl )
            , ( "error", encodeEntropicError e )
            ]
          )
        ]
    By.TxValidationFeeTooSmall _tx required actual -> encodeObject
        [ ( "feeTooSmall", encodeObject
            [ ( "requiredFee", encodeEntropic required )
            , ( "actualFee", encodeEntropic actual )
            ]
          )
        ]
