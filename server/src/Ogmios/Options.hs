...
parserInfo = info (helper <*> parser) $ mempty
    <> progDesc "Ogmios - A JSON-WSP WebSocket adaptor for bcc-node"
    <> header (toString $ unwords
        [ "Provides a bridge between bcc-node and WebSocket clients."
        , "Ogmios translates the existing CBOR-based Shardagnostic mini-protocols"
    ...
    subparser $ command "health-check" $ info (helper <*> parser) $ mempty
        ...
        <> headerDoc (Just $ vsep
            [ string $ toString $ unwords
                ...
                , "and correctly connected to a network / bcc-node."
                ]
            , mempty
            , string $ toString $ unwords
...
parseNetworkParameters configFile = runOrDie $ do
    ...
    let genesisFiles = (,)
            <$> config ^? key "ColeGenesisFile" . _String
            <*> config ^? key "SophieGenesisFile" . _String
    case genesisFiles of
        Nothing ->
            throwE "Missing 'ColeGenesisFile' and/or 'SophieGenesisFile' from Cardano's configuration?"
        Just (toString -> coleGenesisFile, toString -> sophieGenesisFile) -> do
            coleGenesis   <- decodeYaml (replaceFileName configFile coleGenesisFile)
            sophieGenesis <- decodeYaml (replaceFileName configFile sophieGenesisFile)
            let params = (,,)
                    <$> (sophieGenesis ^? key "networkMagic" . _Integer)
                    <*> (iso8601ParseM . toString =<< sophieGenesis ^? key "systemStart" . _String)
                    <*> (coleGenesis ^? key "protocolConsts" . key "k" . _Integer)
            case params of
                Nothing -> do
                    let prettyYaml = decodeUtf8 (Yaml.encodePretty Yaml.defConfig sophieGenesis)
