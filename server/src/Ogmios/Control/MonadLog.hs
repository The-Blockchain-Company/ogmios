...
-- | Working around tbco-monitoring. Ogmios doesn't use 'Severity' from
-- tbco-monitoring because the JSON instances are bonkers and, it defines way
-- too many severity levels. Still, there are places where we need to convert
-- from existing severity types (when wrapping traces from shardagnostic-network in
-- particular).
