...
import Shardagnostic.Network.Protocol.LocalTxMonitor.Client
    ( LocalTxMonitorClient (..) )
import Shardagnostic.Network.Protocol.LocalTxSubmission.Client
    ( LocalTxSubmissionClient (..) )

import qualified Data.Aeson as Json
import qualified Shardagnostic.Consensus.HardFork.Combinator as LSQ
...
-- | A simple wrapper around Shardagnostic 'Clients'. A health check client only
-- carries a chain-sync client.
newtype HealthCheckClient m
    = HealthCheckClient (Clients m Block)

-- | Instantiate a new set of Shardagnostic mini-protocols clients. Note that only
