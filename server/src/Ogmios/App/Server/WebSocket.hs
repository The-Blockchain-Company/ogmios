    ...
    return $ \pending -> unliftIO $ do
        ...
        recordSession sensors $ onExceptions $ acceptRequest pending sub $ \conn -> do
            ...
            withExecutionUnitsEvaluator $ \exUnitsEvaluator exUnitsClients -> do
                withShardagnosticClients tr mode maxInFlight sensors exUnitsEvaluator conn $ \protocolsClients -> do
...
withShardagnosticClients
    ...
    -> (Clients m Block -> m a)
    -> m a
withShardagnosticClients tr mode maxInFlight sensors exUnitsEvaluator conn action = do
