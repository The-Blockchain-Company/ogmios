...
spec = parallel $ do
  ...
  where
    ...
    -- whereas the Shardagnostic framework only allow for finite one. This means
    ...
    -- The Shardagnostic typed protocol follows a strategy of correct-by-construction
...
withChainSyncClient
    :: (MonadSTM m, MonadShardagnostic m)
