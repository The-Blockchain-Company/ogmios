...
import Shardagnostic.Network.Mux
    ( MuxPeer (..), ShardagnosticApplication (..), RunMiniProtocol (..) )
...
type Client m = ShardagnosticApplication
    'InitiatorMode
...
codecs epochSlots nodeToClientV =
  ...
  where
    ...
    cfg = CardanoCodecConfig cole sophie evie jen aurum
      where
        cole   = ColeCodecConfig epochSlots
        sophie = SophieCodecConfig
        evie = SophieCodecConfig
        jen    = SophieCodecConfig
        aurum  = SophieCodecConfig
