...
> ##### 🏢 Server 
>
> - `NextTx` can now take an (optional) extra argument `{ "fields": "all" }` to instrument the server in returning not only a transaction id in `NextTxResponse`, but a full transaction. See [#190](https://github.com/The-Blockchain-Company/ogmios/issues/190).

#### Changed

> ##### 🏢 Server 
>
> - Upgrade internal dependencies to `bcc-node@1.34.1`

> ##### 🚗 TypeScript Client
>
> - ⚠️  Some schema type interface renaming:
>     - `Tx` → `TxCole`
>     - `BlockBodySophie` → `TxSophie`
>     - `BlockBodyAllegra` → `TxAllegra`
>     - `BlockBodyJen` → `TxJen`
>     - `BlockBodyAurum` → `TxAurum`
...
> ##### 🏢 Server 
>
> - Extended the local-tx-submission protocol with a [new `EvaluateTx` query](https://ogmios.dev/mini-protocols/local-tx-submission/#evaluatetx) which evaluates execution units of scripts present in a transaction. This effectively piggybacks on the Aurum's tools from the bcc-ledger while providing a more user-friendly interface regarding network parameters. The API offers well-detailed errors and an interface similar to the `SubmitTx`. See discussion on [#172](https://github.com/The-Blockchain-Company/ogmios/issues/172). 
>
> - New `rewardsProvenance'` query coming as a replacement for the now-deprecated `rewardsProvenance` query. See discussion on [#171](https://github.com/The-Blockchain-Company/ogmios/issues/171).
>
> ##### 🚗 TypeScript Client
>
...
>
>   ⚠️  This new protocol is **NOT** enabled in `bcc-node@1.33.*`. Until its inclusion in a next release, a custom build of bcc-node is required to include a more recent version of `shardagnostic-network` which adds support for that protocol to the Shardagnostic' mini-protocols; namely: [`32af9168`](https://github.com/The-Blockchain-Company/shardagnostic-network/commit/32af9168). 
>
>   A version of `bcc-node@1.33.0` patched with the necessary commits can be found at [The-Blockchain-Company/bcc-node@1.33.0+local-tx-monitor](https://github.com/The-Blockchain-Company/bcc-node/releases/tag/1.33.0+local-tx-monitor).
>
> - New fields in the health object:
>   - `connectionStatus` → `"connected"` or `"disconnected"`, to reflect status with the node. [#154](https://github.com/The-Blockchain-Company/ogmios/issues/154)
>   - `currentEpoch` → which returns the current known epoch of the linked node [#164](https://github.com/The-Blockchain-Company/ogmios/issues/164)
>   - `slotInEpoch` → which returns the relative number of slots elapsed in the current epoch [#164](https://github.com/The-Blockchain-Company/ogmios/issues/154)
>
...
> ##### 🏢 Server 
> 
> - `networkSynchronization` and `currentEra` can be `null` when the server isn't connected to a node. [#154](https://github.com/The-Blockchain-Company/ogmios/issues/154)
> - The `Metrics` trace is now correctly tagged with `MetricsRuntimeStatsDisabled`.
> - Fixed an issue with the Docker monitoring scripts of bcc-node-ogmios, causing issues on restart. [#159](https://github.com/The-Blockchain-Company/ogmios/pulls/159)
> 
>
> ##### 🚗 TypeScript Client
>
...
> ##### 🚗 TypeScript Client
>
> - ⚠️ `Entropic` is now a native BigInt.
>
> - ⚠️ `getServerHealth`'s `connection` argument is now wrapped into an object, mapped to the field `connection`. (see [#135](https://github.com/The-Blockchain-Company/ogmios/issues/135))
> 
...
> ##### 🏢 Server
> 
> - Fixed configuration parsing which would wrongly use Sophie's slots per epoch instead of Cole's. This had an impact on the reported slot number in the chain-sync protocol, where slot numbers would be wrongly offset by `432000` per epoch after the first epoch. 
...
> ##### 🏢 Server
> 
> - Enabled Aurum transaction in the submission protocol (oversight from previous releases). Doing so, explicit compiler warnings have been added to the relevant code to avoid overseeing this in the next upgrade.
> 
...
> 
> - Documented Jen last point to the _points of interests_ for the chain-sync protocol.
>
> - Bumped bcc-node's integration to 1.31.0
> 
> ##### 🚗 TypeScript Client
> 
...
> ##### 🏢 Server
> 
> - Generate and store [test vectors](https://github.com/The-Blockchain-Company/ogmios/tree/master/server/test/vectors) for various JSON requests and responses. This should ease integration for many clients who seek for good coverage of the server inputs/outputs.
> 
...
> 
> - New helpers `isColeEpochBoundaryBlock` and `isColeStandardBlock`.
...
> - Integrated with the Cardano eco-system corresponding to [bcc-node@1.29.0](https://github.com/The-Blockchain-Company/bcc-node/releases/tag/1.29.0) (Aurum!) & latest testnet(s).
>
> - New `aurum` block type, with various additions related to Aurum.
>
> - New state-queries:
> 
>   Query                        | Description
...
> 
> - Added missing properties in Cole's protocol parameters update. Somehow, an `additionalProperties: true` had slipped through and caused the tests to pass with an incomplete schema.
> 
...
> 
> - The Dockerfile now includes a build definition for building `bcc-node` and `ogmios` into the **same image**, which is the default and suggested mode of operation. To build an image with only Ogmios, use the build `--target ogmios`. Docker Hub now hosts two image repositories: `The-Blockchain-Company/bcc-node-ogmios` and `The-Blockchain-Company/ogmios`.
> 
...
> 
> - A new repository for hosting Cardano configurations of various services was created and is now used in Ogmios. Configuration for bcc-node (and therefore Ogmios) or, network genesis can be found in [The-Blockchain-Company/bcc-configurations](https://github.com/The-Blockchain-Company/bcc-configurations). Configurations are updated automatically by a nightly job to be always up-to-date. They can be pulled into projects as git submodules.
> 
> - New possible errors from the transaction submission (stemming from the Aurum integration):
>   - `collateralHasNonBccAssets`
>   - `collateralIsScript`
>   - `collateralTooSmall`
>   - `collectErrors`
>   - `datumsMismatch`
...
> 
> - The TypeScript client is now [fully documented](https://ogmios.dev/api/modules/_bcc_ogmios_client.html)!
...
> 
> - Auxiliary data's `scriptPreImages` in Evie & Jen has been replaced with a field `scripts` which has one field `native`. The value of `native` corresponds to what used to be the value of `scriptPreImages`. In Aurum, `scripts` may also have another field `plutus` with a serialized Plutus script.
> 
...
> 
> - The `TxOut` (and thus Utxo) model definitions have been unified and harmonized across all eras. That is, pre-Jen eras now also wrap Bcc values in an object with a field `"coins": ...`. This reduces the discrepancy between eras for there's now a single TxOut representation valid across all eras. Some fields are however optional and only present in some eras (e.g. `datum` starting from Aurum)
...
> 
> - ⚠️  `SubmitTx` no-longer returns Cole errors. Consequently, submit errors are no longer scoped under `errors.cole` or `errors.sophie` but simply `errors`.
> 
> - ⚠️  Fixed `proposedProtocolParameters` query. All fields are actually required AND, more importantly, it can now return either Sophie protocol parameters or, Aurum protocol parameters.
> 
...
>   - Type `Proposal` renamed into `UpdateProposalSophie`
>   - Types `Utxo1`, `Utxo2`, `UtxoJen` have been unified into a single `Utxo` type. Refer to server breaking changes for details.
...
> 
> - `SubmitTx` can no longer return `SubmitTxError[Cole]`. All the child error types have been removed accordingly, namely:
>   - `UtxoValidationError`
>   - `TxValidationError`
>   - `EntropicError`
...
- Provide missing documentation / JSON-schema for:
  ...
  - Evie & Jen `SubmitTx` failures:
    - Evie:
      ...
      - `outputTooSmall` items are now of type `TxOut[Jen]`
...
- Add missing `mint` field to transaction's body (added since jen).
...
- Implement a 'fast-bech32' encoding library, to speed-up Ogmios serialization of blocks beyond the Sophie era.
...
- Support for the Evie era on the chain-sync, tx submission and state query protocols.
- Support for the Jen era on the chain-sync, tx submission and state query protocols.
...
- Various internal optimization, in particular with rewards to the chain-sync protocol (~14.000 blocks/s in Cole, ~2500 block/s in Sophie and beyond).
...
- Upgraded internal dependencies to the Cardano eco-system working with bcc-node@1.25.1
...
- Support for the Sophie chain in the local-chain-sync protocol.
...
- Several JSON fields renamed to increase consistency between Sophie and Cole.
...
- Initial release and support for:
  - Chain Synchronization (no pipelining between bcc-node & ogmios)
